...
namespace Server.Items
{
	...
	public abstract class BaseHarvestTool : Item, IUsesRemaining, ICraftable
	{
		...
		private class ToggleMiningStoneEntry : ContextMenuEntry
		{
			...
			public ToggleMiningStoneEntry( PlayerMobile mobile, bool valuestone, bool valuegem, int number ) : base( number )
			{
				m_Mobile = mobile;
				...
				if ( mobile.ToggleMiningStone == valuestone || ( valuestone && !stoneMining ) )
					this.Flags |= CMEFlags.Disabled;
				if ( mobile.ToggleMiningGem == valuegem || ( valuegem && !gemMining ) )
					this.Flags |= CMEFlags.Disabled;
			}
