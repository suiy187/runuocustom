			...
			for( int i = 0; i < list.Length; ++i )
			{
				...
				else
				{
					...
					{	
						...
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x41C4 )
						door = new GenericHouseDoor( DoorFacing.EastCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x41C6 )
						door = new GenericHouseDoor( DoorFacing.SouthCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x41C8 )
						door = new GenericHouseDoor( DoorFacing.NorthCW, itemID, 0xEA, 0xF1, false );
					}
					...
					else if( itemID >= 0x4D22 && itemID < 0x4D29 )
					{
						if ( itemID == 0x4D22 )
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x4D24 )
					...
					else if( itemID >= 0x46DD && itemID < 0x46E6 )
					{
						if ( itemID == 0x46DD )
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x46DF )
					...
					else if( itemID >= 0x4372 && itemID < 0x437A )
					{
						if ( itemID == 0x4372 )
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x4374 )
					...
					else if( (itemID >= 0x436E && itemID < 0x4372) || (itemID >= 0x437A && itemID < 0x437E) )
					{
						if ( itemID == 0x436E )
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x4370 )
					...
					else if( itemID >= 0x50D0 && itemID < 0x50DA )
					{
						if ( itemID == 0x50D0 )
						door = new GenericHouseDoor( DoorFacing.WestCCW, itemID, 0xEA, 0xF1, false );
						if ( itemID == 0x50D2 )
		...
		private static int[] m_BlockIDs = new int[]
			{
				...
				0x7A3, 0x7BA, 0x35D2, 0x3609,
        			0x35D2, 0x3609, 0x4317, 0x4318, 0x4317
			};
		...
		private static int[] m_StairSeqs = new int[]
			{
				...
				0x4360, 0x435c, 0x4364, 0x35D4, 0x360B, 
				0x435A, 0x435B, 0x435C, 0x435D, 0x435E,
				0x435F, 0x4360, 0x4361, 0x4362, 0x4363,
				0x4364, 0x4365, 0x4366, 0x4367, 0x4368,
				0x4369, 0x436A, 0x436B, 0x436C, 0x436D
			};
	...
	public class DesignState
	{
		...
		public static bool IsFixture( int itemID )
		{
			...
			#region SA
				...
				return true;
			else if( itemID >= 0x50C8 && itemID < 0x50DA )
				return true;
			#endregion
