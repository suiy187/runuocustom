...
namespace Server.Mobiles
{
	...
	public class DemonKnight : BaseCreature
	{
		...
		public static void DistributeArtifact( BaseCreature creature, bool points, Item artifact )
 		{

			if ( !points )
				DistributeArtifact( FindRandomPlayer( creature ), artifact );
			else
				DistributeArtifact( SortByPoints( creature ), artifact );
 		}
		#endregion
		...
		public static void DistributeArtifact( Mobile to, Item artifact )
		{
            ...
            #region ML
			PlayerMobile pm = to as PlayerMobile;
			if ( pm != null )
				pm.GauntletPoints = 0;
			#endregion
